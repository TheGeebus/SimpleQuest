...
void UActivationGroupSetterNode::ActivateInternal(FGameplayTag InContextualTag)
{
    ...
    // Transient signal: payload threads PendingActivationParams + OriginChain + OriginatingEventID reaching this
    ...
    if (USignalSubsystem* Signals = GI->GetSubsystem<USignalSubsystem>())
    {
        ...
        FQuestActivationGroupTriggeredEvent Event(GroupTag, PendingActivationParams, ProvenanceSource,
            PendingActivationParams.OriginChain, PendingActivationParams.OriginatingEventID);
        ...
        UE_LOG(LogSimpleQuest, Verbose,
            ...
            *PendingActivationParams.OriginatingEventID.AuthoredNodeGuid.ToString(EGuidFormats::Short));
    }
