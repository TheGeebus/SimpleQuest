...
/**
 * Live runtime state of a quest node during PIE, as resolved from WorldState facts by FQuestPIEDebugChannel.
 * Values listed in presentation-priority order — the channel picks the highest-priority state the node currently holds
 * (Blocked > PendingGiver > Active > Completed > Deactivated) so the overlay surfaces the most-urgent-to-see condition.
 */
UENUM()
enum class EQuestNodeDebugState : uint8
{
	...
	/** Runtime instance exists but carries no state fact relevant to the debug overlay. Rare — distinct from Unknown only
		when the node was instantiated by ActivateQuestlineGraph but hasn't yet reached Active. */
	Deactivated,

	/** QuestState.<Tag>.Completed fact is set. */
	Completed,

	/** QuestState.<Tag>.Active fact is set. */
	Active,
