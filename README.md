...
| Q2 2026 | Tag namespace consolidation under `SimpleQuest.*` root — every plugin tag now lives under one parent (`SimpleQuest.Quest.*`, `SimpleQuest.QuestOutcome.*`, `SimpleQuest.QuestState.*`, `SimpleQuest.QuestPrereqRule.*`, `SimpleQuest.QuestActivationGroup.*`, `SimpleQuest.QuestChannel.*`); comprehensive `GameplayTagRedirects` for transparent migration of existing authored references; pre-existing LinkedQuestline-prereq compilation bug fixed as collateral | **Shipped** (v0.4.0) |
