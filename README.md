...
- **Stale Quest Tags** (Window > Developer Tools > Debug) — nomad tab listing quest-component tag references whose target isn't registered in the runtime tag manager. Refresh stays at the sub-second loaded-levels scan; **Full Project Scan** fans out to Actor Blueprint defaults and unloaded levels (with comprehensive-vs-class-filtered World Partition coverage). Per-row Source badge (*Loaded* / *BP CDO* / *Unloaded*) drives a matching navigation affordance: **Find** frames a loaded actor, **Open BP** opens a Blueprint editor, **Open Level** loads a containing umap. Per-row Clear works on every source; affected packages roll into a panel-header **Save All Modified** button. Filter bar + sortable columns. Headless equivalent ships as a commandlet — `UnrealEditor-Cmd.exe <project>.uproject -run=StaleQuestTagsScan -OutputJson=<path>` writes structured JSON and returns `0` (clean) / `1` (stale found) / `2` (infra failure) for CI gating; a Windows `.bat` helper at `Scripts/RunStaleQuestTagsScan.bat` wraps the invocation.
