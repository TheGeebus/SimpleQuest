...
- **Advancement holds** — quest flow normally continues the instant a step completes, leaving nowhere to put an outro line or a reward popup. Place a hold when a step finishes, play whatever should play, and release it when you're done. Holds compose, so an audio hold and a cutscene hold don't cancel each other, and one nobody releases reports itself rather than stalling a questline in silence. The framework owns suspend and resume, and your game owns the pacing.
- **Rewards as adapters** — self-configuring reward types read completion context and broadcast to any actor whose Reward Recipient component watches that reward type. The granting node never needs to know who's listening. A preview query returns what a completion advertises *before* it's earned: the "do this, get this" data a quest-giver hub or bounty board needs. Rewards compose rather than multiply: define a bundle once in a Reward Set and reference it anywhere, include one set inside another, and attach modifiers that scale, cap, redirect or withhold a grant - so a transform written once applies to XP, currency and loot alike.
...
The reference set is expected to expand in future versions with timer, world-state-fact, and composite (ANY/ALL wrapper) objectives.
...
Future versions ship reference implementations for the timer, world-state-fact, and composite patterns. Until then they're straightforward to write against the base class.
...
| Q3 2026  | Pluggable data resolver — user-writable adapters for bidirectional graph ↔ text data pipelines; version-controllable, diff-friendly progression data outside .uassets, fitting existing studio content workflows                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | **Shipped** (v0.7.0 - v0.7.2)      |
| Q3 2026  | Composition and control — resolver export, plan, and apply on the public module API for programmatic and tool-driven authoring; advancement holds so presentation can pace quest flow; reward sets that can include other sets, and modifiers that transform a grant without writing a new reward class                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | **Shipped** (v0.8.0)               |
...
| Q1 2027  | Sample project + expanded objective library — a complete playable progression exercising branching outcomes, prerequisites, linked questlines, rewards, and save/load end to end; timer, world-state-fact, and composite reference objectives built alongside it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Planned (v0.9.0)                   |
| Q2 2027  | Documentation pass — full authoring and API documentation grounded in the sample project's real usage                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Planned (v0.10.0)                  |
