...
| Q2 2026 | Tag namespace consolidation under `SimpleQuest.*` root + giver-gated quest prereq evaluation fix — every plugin tag now lives under one parent (`SimpleQuest.Quest.*`, `SimpleQuest.QuestOutcome.*`, `SimpleQuest.QuestState.*`, `SimpleQuest.QuestPrereqRule.*`, `SimpleQuest.QuestActivationGroup.*`, `SimpleQuest.QuestChannel.*`); comprehensive `GameplayTagRedirects` for transparent migration; LinkedQuestline-prereq compilation bug fix; giver-side prereq evaluation gating | In development (v0.4.0) |
