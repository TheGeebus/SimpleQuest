...
- **`USignalSubsystem`** — hierarchical tag router with complex-tag-tree deduplication. Subscribers bound to a parent tag receive events from every descendant, with the event payload reporting the most-specific tag that matched their binding. When a single logical event publishes under multiple tag paths simultaneously — e.g., a quest node carrying both its canonical tag and its alias forms inside each enclosing linked questline asset — the bus collapses them into a single event instance per subscriber, rather than firing one per matching path. Channels are keyed by tag and payload type; payloads are plain USTRUCTs delivered via `FInstancedStruct`. Complements `GameplayMessageRouter`: GMR excels at flat tag-keyed message routing, while `USignalSubsystem` handles cases where a hierarchical tag tree describes structurally-identified events whose multiple namings shouldn't fan out as duplicate broadcasts.
...
- **Stale Quest Tags** (Window > Developer Tools > Debug) — nomad tab listing quest-component tag references whose target isn't registered in the runtime tag manager. **Scan Loaded** stays at the sub-second loaded-levels scan; **Full Project Scan** fans out to Actor Blueprint defaults and unloaded levels (with comprehensive-vs-class-filtered World Partition coverage). Per-row Source badge (*Loaded* / *BP CDO* / *Unloaded*) drives a matching navigation affordance: **Find** frames a loaded actor, **Open BP** opens a Blueprint editor, **Open Level** loads a containing umap. Per-row Clear works on every source; **multi-select + Clear Selected (N)** wraps a confirmation dialog (with per-source breakdown) and an atomic single-transaction undo around the bulk mutation; affected packages roll into a panel-header **Save All Modified** button. Sortable + filterable columns include Source, **Level** (umap leaf for Loaded/Unloaded; em-dash for BP CDOs), Actor, Component, Field, and Stale Tag. Headless equivalent ships as a commandlet — `UnrealEditor-Cmd.exe <project>.uproject -run=StaleQuestTagsScan -OutputJson=<path>` writes structured JSON and returns `0` (clean) / `1` (stale found) / `2` (infra failure) for CI gating; a Windows `.bat` helper at `Scripts/RunStaleQuestTagsScan.bat` wraps the invocation.
